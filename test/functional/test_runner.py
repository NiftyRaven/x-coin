...
def main():
    ...
    # Check that the build was configured with wallet, utils, and xcoind
    ...
    if not (enable_wallet and enable_cli and enable_ravend):
        print("No functional tests to run. Wallet, utils, and xcoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    ...
    #Set env vars
    if "XCOIND" not in os.environ:
        os.environ["XCOIND"] = build_dir + '/src/xcoind' + exeext
        os.environ["RAVENCLI"] = build_dir + '/src/xcoin-cli' + exeext
