...
class AbandonConflictTest(RavenTestFramework):
    ...
    def run_test(self):
        ...
        # Verify that B and C's 10 XFER outputs are available for spending again because AB1 is now conflicted
        new_balance = self.nodes[0].getbalance()
        assert_equal(new_balance, balance + Decimal("20"))
        balance = new_balance

        # There is currently a minor bug around this and so this test doesn't work.  See Issue #7315
        # Invalidate the block with the double spend and B's 10 XFER output should no longer be available
