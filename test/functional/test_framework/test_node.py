...
class TestNode:
    ...
    def __init__(self, i, dirname, extra_args, rpchost, timewait, binary, stderr, mocktime, coverage_dir):
        ...
        if timewait:
            ...
            self.rpc_timeout = 60
        if binary is None:
            self.binary = os.getenv("XCOIND", "xcoind")
        else:
            self.binary = binary
        self.stderr = stderr
    ...
    def wait_for_rpc_connection(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            assert self.process.poll() is None, "xcoind exited with status %i during initialization" % self.process.returncode
            try:
        ...
        raise AssertionError("Unable to connect to xcoind")
...
class TestNodeCLI:
    ...
    def __call__(self, *args, input_data=None):
        # TestNodeCLI is callable with xcoin-cli command-line args
