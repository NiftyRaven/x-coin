...
1. Clone the X Coin source code:
    ```shell
    git clone https://github.com/NiftyRaven/x-coin
    cd X Coin
    ```
...
## `disable-wallet` mode
When the intention is to run only a P2P node without a wallet, X Coin may be
...
## Running
X Coin is now available at `./src/xcoind`
...
The first time you run xcoind, it will start downloading the blockchain. This process could
...
-------
    ...
    ./src/xcoind -daemon # Starts the raven daemon.
