...
The X Coin Doxygen documentation can be generated by running `make docs` from the top-level 
...
X Coin is a multithreaded application, and deadlocks or other multithreading bugs
...
However, in open source software such as X Coin, where everyone uses
...
pay attention to for reviewers of X Coin code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: X Coin uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against X Coin so that they can be integrated
