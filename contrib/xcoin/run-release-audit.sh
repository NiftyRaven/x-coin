...
LINUX_TAR="$ROOT/dist/xcoin-1.0.8-linux-x86_64.tar.gz"
WIN_ZIP="$ROOT/dist/xcoin-1.0.8-win-x86_64.zip"
mkdir -p "$LOGDIR"
: > "$LOGDIR/summary.tsv"
...
if not body.startswith("# X Coin"):
    ...
    sys.exit(1)
links = re.findall(r"\[[^\]]*\]\((https://github.com/NiftyRaven/x-coin/releases/download/v1\.0\.6/[^)]+)\)", text)
need = [
    "https://github.com/NiftyRaven/x-coin/releases/download/v1.0.8/X-Coin-1.0.8-Windows.zip",
    "https://github.com/NiftyRaven/x-coin/releases/download/v1.0.8/X-Coin-1.0.8-Linux-x86_64.tar.gz",
]
if links[:2] != need:
...
for i, line in enumerate(text.splitlines(), 1):
    ...
    if re.match(r"addnode\s*=", s, re.I):
        ...
        sys.exit(1)
    m = re.match(r"xoauthclientid\s*=\s*(.*)$", s, re.I)
    if m:
        val = m.group(1).strip()
        if val != ALLOWED_CLIENT_ID:
            print("unexpected Client ID on line %d: %s" % (i, s))
            sys.exit(1)
        saw_client = True
        continue
    if re.match(r"dnsseed\s*=\s*1", s, re.I):
        ...
        sys.exit(1)
if not saw_client:
    print("package xcoin.conf must bake the operator Client ID")
    sys.exit(1)
...
PY
then
  record PASS package-conf-no-invented-values "listen on; operator Client ID baked; addnode commented"
else
  record FAIL package-conf-no-invented-values "shipped xcoin.conf invents a seed or DNS seed, or Client ID is not the operator bake"
fi
...
if [[ -f "$LINUX_TAR" ]]; then
  ...
  if [[ -x "$LROOT/X Coin Wallet" && -x "$LROOT/X Coin Practice Wallet" && -x "$LROOT/bin/xcoin-qt" && -f "$LROOT/xcoin.conf" ]]; then
    ...
    while IFS= read -r p; do
      [[ -z "$p" ]] && continue
      # expect xcoin-1.0.8-linux-x86_64/X Coin Wallet
      slashes="${p//[^\/]/}"
...
echo "== GitHub Release v1.0.8 (non-draft, both wallet assets) =="
if command -v gh >/dev/null 2>&1; then
  if python3 - <<'PY'
import json, subprocess, sys
raw = subprocess.check_output(
    ["gh", "api", "repos/NiftyRaven/x-coin/releases/tags/v1.0.8"],
    text=True,
)
j = json.loads(raw)
if j.get("draft") is True:
    print("v1.0.8 is still a draft")
    sys.exit(1)
if j.get("prerelease") is True:
    print("v1.0.8 is marked prerelease")
    sys.exit(1)
names = {a.get("name") for a in j.get("assets") or []}
need = {"X-Coin-1.0.8-Windows.zip", "X-Coin-1.0.8-Linux-x86_64.tar.gz"}
if not need.issubset(names):
    print("missing assets, have %r" % sorted(names))
    sys.exit(1)
sizes = {a["name"]: a.get("size") for a in j.get("assets") or []}
if sizes.get("X-Coin-1.0.8-Windows.zip", 0) < 1_000_000:
    print("Windows zip too small: %s" % sizes.get("X-Coin-1.0.8-Windows.zip"))
    sys.exit(1)
if sizes.get("X-Coin-1.0.8-Linux-x86_64.tar.gz", 0) < 1_000_000:
    print("Linux tar.gz too small: %s" % sizes.get("X-Coin-1.0.8-Linux-x86_64.tar.gz"))
    sys.exit(1)
...
PY
  then
    ...
    record FAIL github-release-v1 "v1.0.8 missing, draft, or missing wallet assets"
  fi
else
...

{
  ...
  echo "| Asked | Result |"
  echo "| --- | --- |"
  cov_row "Existing smokes on regtest" "smoke-regtest smoke-xsession smoke-eligibility smoke-gossip smoke-package-conf smoke-gui smoke-benchmark smoke-isolation smoke-sabotage smoke-extra smoke-abuse"
  ...
  cov_row "Session file / mock mismatch is not signed in (send still works)" "smoke-abuse smoke-xsession"
  cov_row "GitHub default-branch README is download-first" "github-homepage-readme"
  cov_row "GitHub Release v1.0.8 is published with both wallet assets" "github-release-v1"
  echo
  echo "## Packages"
  echo
  echo "| Archive | Open this |"
  echo "| --- | --- |"
  echo "| \`dist/xcoin-1.0.8-linux-x86_64.tar.gz\` | **X Coin Wallet** (ELF) in the first unpacked folder |"
  echo "| \`dist/xcoin-1.0.8-win-x86_64.zip\` | **X Coin Wallet.exe** in the first unpacked folder |"
  echo
  echo "GitHub Release assets (the download): \`X-Coin-1.0.8-Windows.zip\` and"
  echo "\`X-Coin-1.0.8-Linux-x86_64.tar.gz\` on tag \`v1.0.8\`."
  echo
  echo "## Could not run"
  echo
...
if [[ "$hp" == "PASS" && "$rel" == "PASS" ]]; then
  ...
  visit="NO — launch blocker: GitHub Release v1.0.8 is missing, draft, or missing wallet assets."
else
...
fi
{
  ...
  echo "2. Click **Download Windows wallet** or **Download Linux wallet** (GitHub Release v1.0.8)."
  ...
  cov_row "Typed handle cannot claim Verified X or someone else's root" "smoke-abuse smoke-xsession smoke-extra"
  cov_row "Unsigned sendraw decode-fails; handle steal rejected" "smoke-sabotage"
  ...
  cov_row "Operator invite list cannot exclude a verified wallet" "smoke-abuse smoke-eligibility"
  cov_row "Tampered xsession.json / foreign session file is not signed in (send still works)" "smoke-abuse"
  echo
