...
Source10:	https://raw.githubusercontent.com/raven/raven/v%{version}/contrib/debian/examples/xcoin.conf

#man pages
Source20:	https://raw.githubusercontent.com/raven/raven/v%{version}/doc/man/xcoind.1
Source21:	https://raw.githubusercontent.com/raven/raven/v%{version}/doc/man/xcoin-cli.1
Source22:	https://raw.githubusercontent.com/raven/raven/v%{version}/doc/man/xcoin-qt.1

#selinux
Source30:	https://raw.githubusercontent.com/raven/raven/v%{version}/contrib/rpm/raven.te
# Source31 - what about xcoin-tx and bench_raven ???
...
The xcoin-cli utility allows you to communicate and control a raven daemon
over RPC, the xcoin-tx utility allows you to create a custom transaction, and
...
cp -p %{SOURCE10} ./xcoin.conf.example
tar -zxf %{SOURCE1}
...
mv %{buildroot}%{_bindir}/xcoind %{buildroot}%{_sbindir}/xcoind

# systemd stuff
mkdir -p %{buildroot}%{_tmpfilesdir}
cat <<EOF > %{buildroot}%{_tmpfilesdir}/xcoin.conf
d /run/xcoind 0750 raven raven -
EOF
touch -a -m -t 201504280000 %{buildroot}%{_tmpfilesdir}/xcoin.conf
...
CONFIG_FILE="%{_sysconfdir}/raven/xcoin.conf"
DATA_DIR="%{_localstatedir}/lib/raven"
PID_FILE="/run/xcoind/xcoind.pid"
EOF
...
[Service]
Type=forking
ExecStart=%{_sbindir}/xcoind -daemon -conf=\${CONFIG_FILE} -datadir=\${DATA_DIR} -pid=\${PID_FILE} \$OPTIONS
...
# man pages
install -D -p %{SOURCE20} %{buildroot}%{_mandir}/man1/xcoind.1
install -p %{SOURCE21} %{buildroot}%{_mandir}/man1/xcoin-cli.1
%if %{_buildqt}
install -p %{SOURCE22} %{buildroot}%{_mandir}/man1/xcoin-qt.1
%endif
...
%check
make check
srcdir=src test/xcoin-util-test.py
...
%doc COPYING xcoin.conf.example doc/README.md doc/bips.md doc/files.md doc/multiwallet-qt.md doc/reduce-traffic.md doc/release-notes.md doc/tor.md
%attr(0755,root,root) %{_bindir}/xcoin-qt
...
%attr(0644,root,root) %{_mandir}/man1/xcoin-qt.1*
%endif
...
%defattr(-,root,root,-)
%license COPYING db-%{bdbv}.NC-LICENSE
%doc COPYING xcoin.conf.example doc/README.md doc/REST-interface.md doc/bips.md doc/dnsseed-policy.md doc/files.md doc/reduce-traffic.md doc/release-notes.md doc/tor.md
%attr(0755,root,root) %{_sbindir}/xcoind
%attr(0644,root,root) %{_tmpfilesdir}/xcoin.conf
...
%attr(0644,root,root) %{_mandir}/man1/xcoind.1*

%files utils
%defattr(-,root,root,-)
%license COPYING
%doc COPYING xcoin.conf.example doc/README.md
%attr(0755,root,root) %{_bindir}/xcoin-cli
%attr(0755,root,root) %{_bindir}/xcoin-tx
%attr(0755,root,root) %{_bindir}/bench_raven
%attr(0644,root,root) %{_mandir}/man1/xcoin-cli.1*
