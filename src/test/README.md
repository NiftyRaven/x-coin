...
By default the log messages from the X Coin application are not echoed 
when running the unit tests.  If it is desired to print this log data change 
the following from 'false' to 'true' in the `test_xcoin.cpp` file and uncomment
three lines in the `script\interpreter.cpp\ VerifyScript` method and recompile:

    src\test\test_xcoin.cpp:
...
Run `test_xcoin --list_content` to get a full list of available unit tests.

To run just the 'getarg_tests' (verbosely):

    test_xcoin --run_test=getarg_tests

... or to run just the doubledash test:

    test_xcoin --run_test=getarg_tests/doubledash_test
