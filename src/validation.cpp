...
#if defined(NDEBUG)
# error "X Coin cannot be compiled without assertions."
#endif
...
void UpdateCoins(const CTransaction& tx, CCoinsViewCache& inputs, CTxUndo &txundo, int nHeight, uint256 blockHash, CAssetsCache* assetCache, std::pair<std::string, CBlockAssetUndo>* undoAssetData)
{
    ...
    // add outputs
    AddCoins(inputs, tx, nHeight, blockHash, false, assetCache, undoAssetData); /** XCOIN START */ /* Pass assetCache into function */ /** XCOIN END */
}
...
static DisconnectResult DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, CAssetsCache* assetsCache = nullptr, bool ignoreAddressIndex = false, bool databaseMessaging = true, bool fJustCheck = false)
{
    bool fClean = true;
...
//                                      << ", " << i << ", " << hash.GetHex() << ", " << k << ", " << "true" << ", " << assetAmount << std::endl;
        ...
        for (size_t o = 0; o < tx.vout.size(); o++) {
            if (!tx.vout[o].scriptPubKey.IsUnspendable()) {
                ...
                Coin coin;
                bool is_spent = view.SpendCoin(out, &coin, &tempCache); /** XCOIN START */ /* Pass assetsCache into the SpendCoin function */ /** XCOIN END */
        ...
        if (AreAssetsDeployed()) {
            if (assetsCache) {
                if (tx.IsNewAsset()) {
                    ...
                    // VerifyDB level 3 dry-disconnects the last -checkblocks
                    // (default 6) and does not reconnect. The assignment map
                    // and assets DB are process-global, unlike CAssetsCache
                    // which VerifyDB copies. A live Remove here dropped a
                    // confirmed handle → root after every xcoind restart.
                    std::string xid;
                    if (!fJustCheck && ParseXAccountAssignment(tx, xid))
...
bool static FlushStateToDisk(const CChainParams& chainparams, CValidationState &state, FlushStateMode mode, int nManualPruneHeight) {
    ...
    try {
    {
        ...
        if (fDoFullFlush) {
            ...
            if (!CheckDiskSpace((48 * 2 * 2 * pcoinsTip->GetCacheSize()) + assetDirtyCacheSize * 2)) /** XCOIN START */ /** XCOIN END */
                return state.Error("out of disk space");
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (!ConnectBlock(block, state, &indexDummy, viewNew, chainparams, &assetCache, true)) /** XCOIN START */ /*Add asset to function */ /** RVN END*/
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        if (nCheckLevel >= 3 && pindex == pindexState && (coins.DynamicMemoryUsage() + pcoinsTip->DynamicMemoryUsage()) <= nCoinCacheUsage) {
            ...
            DisconnectResult res = DisconnectBlock(block, pindex, coins, &assetCache, true, false, true);
...
bool AreEnforcedValuesDeployed()
{
    ...
    // New genesis: X Coin BIP9 windows are in the past. Keep asset rules on.
