...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-assetindex", strprintf(_("Keep an index of assets, used by listaddressesbyasset / explorers (default: %u). Changing requires -reindex."), DEFAULT_ASSETINDEX));
    ...
    if (showDebug)
    {
        strUsage += HelpMessageOpt("-printpriority", strprintf("Log transaction fee per kB when assembling lottery blocks (default: %u)", DEFAULT_PRINTPRIORITY));
    }
    ...
    if (showDebug) {
        strUsage += HelpMessageOpt("-acceptnonstdtxn", strprintf("Relay and include \"non-standard\" transactions in lottery blocks (%sdefault: %u)", "testnet/regtest only; ", !testnetChainParams->RequireStandard()));
        strUsage += HelpMessageOpt("-incrementalrelayfee=<amt>", strprintf("Fee rate (in %s/kB) used to define cost of relay, used for mempool limiting and BIP 125 replacement. (default: %s)", CURRENCY_UNIT, FormatMoney(DEFAULT_INCREMENTAL_RELAY_FEE)));
        strUsage += HelpMessageOpt("-dustrelayfee=<amt>", strprintf("Fee rate (in %s/kB) used to defined dust, the value of an output such that it will cost more than its value in fees at this fee rate to spend it. (default: %s)", CURRENCY_UNIT, FormatMoney(DUST_RELAY_TX_FEE)));
    }
    strUsage += HelpMessageOpt("-bytespersigop", strprintf(_("Equivalent bytes per sigop in transactions for relay and lottery block assembly (default: %u)"), DEFAULT_BYTES_PER_SIGOP));
    strUsage += HelpMessageOpt("-datacarrier", strprintf(_("Relay and include data carrier transactions in lottery blocks (default: %u)"), DEFAULT_ACCEPT_DATACARRIER));
    strUsage += HelpMessageOpt("-datacarriersize", strprintf(_("Maximum size of data in data carrier transactions we relay and include in lottery blocks (default: %u)"), MAX_OP_RETURN_RELAY));
    strUsage += HelpMessageOpt("-mempoolreplacement", strprintf(_("Enable transaction replacement in the memory pool (default: %u)"), DEFAULT_ENABLE_REPLACEMENT));
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in %s/kB) smaller than this are considered zero fee for relaying, lottery block assembly and transaction creation (default: %s)"),
    ...
    strUsage += HelpMessageOpt("-xlookupbearer=<token>", _("Seed only. X API app bearer used to check that a handle is a live blue check. Or set XCOIN_X_BEARER. Never put this in the user wallet package. Seed does not Sign in."));
    strUsage += HelpMessageOpt("-xattestorkey=<file>", _("Seed only. Private key that stamps blue-check eligibility and signs XSD1 (default: datadir/xattestor.key). Must match the baked consensus pubkey. No Sign-in on the seed."));
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    boost::signals2::connection genesis_conn;
    if (chainActive.Tip() == nullptr) {
        genesis_conn = uiInterface.NotifyBlockTip.connect(BlockNotifyGenesisWait);
    } else {
        fHaveGenesis = true;
    }
    ...
    // Wait for genesis block to be processed
    {
        ...
        if (genesis_conn.connected())
            genesis_conn.disconnect();
    }
