...
UniValue viewallmessages(const JSONRPCRequest& request) {
    ...
    if (!fMessaging) {
        ...
        ret.push_back("Messaging is disabled. To enable messaging, run the wallet without -disablemessaging or remove disablemessaging from your xcoin.conf");
        return ret;
    }
...
UniValue subscribetochannel(const JSONRPCRequest& request) {
    ...
    if (!fMessaging) {
        throw JSONRPCError(RPC_DATABASE_ERROR, "Messaging is disabled. To enable messaging, run the wallet without -disablemessaging or remove disablemessaging from your xcoin.conf");
    }
