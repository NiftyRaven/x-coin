...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total X Coin balance of the wallet\n"
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given X Coin address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The X Coin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The X Coin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: X Coin address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are X Coin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) X Coin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The X Coin address to use for the signature.\n"
...
UniValue getaddressmempool(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  {\n"
            "    \"address\"  (string) The base58check encoded address\n"
            "    \"assetName\"  (string) The name of the associated asset (XFER for the native coin)\n"
...
UniValue getaddressutxos(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"assetName\"   (string, optional) Get UTXOs for a particular asset instead of XFER ('*' for all assets).\n"
            "}\n"
            "\nResult\n"
            "[\n"
            "  {\n"
            "    \"address\"  (string) The address base58check encoded\n"
            "    \"assetName\" (string) The asset associated with the UTXOs (XFER for the native coin)\n"
    ...
    for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> >::const_iterator it=unspentOutputs.begin(); it!=unspentOutputs.end(); it++) {
        ...
        std::string assetNameOut = RVN; // "XFER" — identifier name RVN kept
        if (assetName != RVN) {
            CAmount _amount;
...
UniValue getaddressdeltas(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1 || !request.params[0].isObject())
        throw std::runtime_error(
            ...
            "  \"assetName\"   (string, optional) Get deltas for a particular asset instead of XFER.\n"
            "}\n"
            "\nResult:\n"
            "[\n"
            "  {\n"
            "    \"assetName\"  (string) The asset associated with the deltas (XFER for the native coin)\n"
            "    \"satoshis\"  (number) The difference of satoshis\n"
            "    \"txid\"  (string) The related txid\n"
            "    \"index\"  (number) The related input or output index\n"
            "    \"height\"  (number) The block height\n"
...
UniValue getaddressbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"assetName\"  (string) The asset associated with the balance (XFER for the native coin)\n"
            "    \"balance\"  (string) The current balance in satoshis\n"
            "    \"received\"  (string) The total number of satoshis received (including change)\n"
            "  },...\n"
            "\n]"
