...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "3. maxtries     (numeric, optional) Unused (kept for RPC compatibility).\n"
            "\nResult:\n"
...
UniValue estimatefee(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("estimatefee")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "estimatefee is deprecated and will be fully removed in v0.17. "
            "To use estimatefee in v0.16, restart xcoind with -deprecatedrpc=estimatefee.\n"
...
UniValue getgenerate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            "getgenerate\n"
            "\nRemoved. X Coin does not mine. Use getlotteryinfo.\n"
        );
    throw JSONRPCError(RPC_METHOD_NOT_FOUND, "PoW mining removed; use getlotteryinfo");
}
...
UniValue setgenerate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            "setgenerate\n"
            "\nRemoved. X Coin does not mine. A running node is an active lottery node.\n"
        );
    throw JSONRPCError(RPC_METHOD_NOT_FOUND, "PoW mining removed; lottery producer starts with the node");
}
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* Coin generation (regtest / on-demand assembly; not PoW) */
