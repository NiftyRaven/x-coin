...
void ReissueAssetDialog::CheckFormState()
{
    ...
    if (!ui->addressText->text().isEmpty()) {
        if (!IsValidDestination(dest)) {
            showMessage(tr("Invalid X Coin destination address"));
            return;
        }
    }
    ...
    if (fReissuingRestricted) {
        ...
        if (!strVerifier.empty()) {
            ...
            if (fHasQuantity && !IsValidDestination(dest)) {
                ui->addressText->setStyleSheet(STYLE_INVALID);
                showMessage(tr("Warning: Invalid X Coin address"));
                return;
            }
...
void ReissueAssetDialog::onReissueAssetClicked()
{
    ...
    QString amount = "<b>" + QString::fromStdString(ValueFromAmountString(GetReissueAssetBurnAmount(), 8)) + " XFER";
    amount.append("</b>");
...
void ReissueAssetDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid X Coin address"));
        }
        else // Valid address
        {
