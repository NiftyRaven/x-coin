...
/**
 * X Coin lottery pool advert (public name + member payout scripts + signature).
 * Password is never on the wire.
 */
extern const char *XPL;
};
...
enum ServiceFlags : uint64_t {
    ...
    // set by all X Coin nodes, and is unset by SPV clients or other peers that just want
    // network services but don't provide them.
    NODE_NETWORK = (1 << 0),
    // NODE_GETUTXO means the node is capable of responding to the getutxo protocol request.
    // X Coin does not support this but a patch set called Raven XT does.
    // See BIP 64 for details on how this is implemented.
    NODE_GETUTXO = (1 << 1),
    // NODE_BLOOM means the node is capable and willing to handle bloom-filtered connections.
    // X Coin nodes used to support this by default, without advertising this bit,
