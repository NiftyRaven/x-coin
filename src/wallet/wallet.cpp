...

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if (fGetRVN) { // Looking for XFER Tx OutPoints Only
                    if (fRVNLimitHit) // We hit our limit
                        continue;

                    // We only want XFER OutPoints. Don't include Asset OutPoints
                    if (isAssetScript)
                        continue;
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                for (const auto& recipient : vecSend)
                {
                    ...
                    if (IsDust(txout, ::dustRelayFee) && !IsScriptTransferAsset(recipient.scriptPubKey)) /** XCOIN START */ /** XCOIN END */
                    {
